...
#define CS_HIGH()	{HAL_GPIO_WritePin(PIN_CSSD_GPIO_Port, PIN_CSSD_Pin, GPIO_PIN_SET);}
#define CS_LOW()	{HAL_GPIO_WritePin(PIN_CSSD_GPIO_Port, PIN_CSSD_Pin, GPIO_PIN_RESET);}
...
)
{
	...
	HAL_SPI_TransmitReceive(&SD_SPI_HANDLE, buff, buff, btr, 100);
}
...
//		xchg_spi(*(buff+i));
//	}
	//HAL_SPI_Transmit(&SD_SPI_HANDLE,(uint8_t *) buff, btx, 100);
	HAL_SPI_Transmit_DMA(&SD_SPI_HANDLE,(uint8_t *) buff, btx);
}
#endif
...
)
{
	...
	if (send_cmd(CMD0, 0) == 1) {			/* Put the card SPI/Idle state */
		...
		if (send_cmd(CMD8, 0x1AA) == 1) {	/* SDv2? */
			...
			dbgprintf("OCR1: 0x%02X 0x%02X 0x%02X 0x%02X\n",ocr[0],ocr[1],ocr[2],ocr[3]);
			if (ocr[2] == 0x01 && ocr[3] == 0xAA) {				/* Is the card supports vcc of 2.7-3.6V? */
				while (SPI_Timer_Status() && send_cmd(ACMD41, 1UL << 30)) ;	/* Wait for end of initialization with ACMD41(HCS) */
				if (SPI_Timer_Status() && send_cmd(CMD58, 0) == 0) {		/* Check CCS bit in the OCR */
					for (n = 0; n < 4; n++) ocr[n] = xchg_spi(0xFF);
					dbgprintf("OCR2: 0x%02X 0x%02X 0x%02X 0x%02X\n",ocr[0],ocr[1],ocr[2],ocr[3]);
